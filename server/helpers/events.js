...
const events = async (req, res) => {
  ...
  const event = ObjectId.isValid(slug) ? await  collection.find({_id:ObjectId(slug)}).toArray() : await collection.find({slug}).toArray() 
  send(res, 200, event)
}
...
module.exports = { events: cors(events), create, eventsByOrganizer:cors(eventsByOrganizer) }