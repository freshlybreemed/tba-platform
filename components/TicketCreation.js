        ...
        <Form layout="inline">
          ...
          <Form.Item label="Price">
            {getFieldDecorator('price', {
                initialValue: 0
            })(<InputNumber min={0} disabled={getFieldValue("type")==='free'} parser={value => value.replace(/\$\s?|(,*)/g, '')} formatter={value => `$ ${value}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} />)}
          </Form.Item>
...
class TicketCreation extends React.Component {
  ...
  handleCreate = () => {
    ...
    form.validateFields((err, values) => {
      ...
      this.setState({ visible: false, tixArray }, ()=> console.log(this.state))
    });
