...
class Create extends React.Component {
  ...
  handleTimeChange(end, start, val){
    this.setState({ isBefore: end.isAfter(start)},() => console.log(this.state.isBefore))
  }
  render() {
    ...
    return (
      <Form onSubmit={this.handleSubmit}>
        <FormItem {...formItemLayout} label="Event Title">
         <Input  {...getFieldProps('createdEventForm.title', {
            getValueFromEvent: (e) => {
              this.props.dispatch({
                type:'save_fields',
                payload: {
        ...
        {this.state.ready? 
          <PlacesAutocomplete
            value={typeof formState.location !== 'undefined'? formState.location.name: ""}
        ...
        <Form.Item  {...formItemLayout} label="Start Time" style={{ marginBottom: 0 }}>
          ...
          <FormItem style={{ display: 'inline-block', width: 'calc(50% - 12px)' }}>
            <TimePicker {...getFieldProps('createdEventForm.startTime', {
              ...
              })}minuteStep={15} allowClear={false} use12Hours defaultOpenValue={moment("12:00 AM", "h:mm A")} format="h:mm a"  />
          </FormItem>
          </Form.Item>
            ...
            <DatePicker {...getFieldProps('createdEventForm.endDate', {
                ...
                disabledDate={(current)=> {
                  if (!getFieldValue('createdEventForm.startDate')) return ''
                    console.log('startdate', getFieldValue('createdEventForm.startDate'))
                    console.log('current', current)
                    return current && current.valueOf() < getFieldValue('createdEventForm.startDate')}
              }/>
          </Form.Item>
        <span style={{ display: 'inline-block', width: '24px', textAlign: 'center' }}>-</span>
          <FormItem validateStatus={getFieldError('createdEventForm.endTime') ? "error" :""} help={getFieldError('createdEventForm.endTime') ? "End time must be after start time!":""} style={{ display: 'inline-block', width: 'calc(50% - 12px)' }}>
            ...
            {...getFieldProps('createdEventForm.endTime', {
              ...
              getValueFromEvent: (e) => {
               ...
               console.log(e)
               console.log('error?',getFieldError('createdEventForm.endTime') )
               if (moment.isMoment(momento)){
                 e.set('year', momento.year())
                 e.set('month', momento.month())
                 e.set('date', momento.date())
          ...
          {ReactQuill ? <FormItem {...formItemLayout} label="Event Description">
            <ReactQuill  value={this.props.formState.description.value} name="createdEventForm.description" onChange={this.handleChange} />
        </FormItem>: ''}
        <FormItem
          {...formItemLayout}
          label={
        ...
        <FormItem {...formItemLayout} label="Upload Event Image">
            <Upload {...getFieldProps('createdEventForm.image', {
              rules: [{ required: true, message: 'Upload event image!' }]
            })
          } image={createdEvent.image} saveImage={this.saveImage} />
