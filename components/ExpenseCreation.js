...
class ExpenseCreation extends React.Component {
  state = {
    visible: false,
    ticketTypes: {},
    expenseArray: []
  ...
  removeTicket = tix => {
    ...
    const expenseArray = this.state.expenseArray.filter(ticket => {
      return ticket.name !== tix;
    });
    let expenses = {};
    expenseArray.forEach(ticket => {
      expenses[ticket.name] = ticket;
    });
    this.setState({ expenseArray }, () => console.log(this.state));
  };
  saveFormRef = formRef => {
...
ExpenseCreateForm = connect(state => {
  return {};
})(
  createForm({
...
ExpenseCreation = connect(state => {
  return {
    expenses: state.selectedEvent.expenses
  };
})(ExpenseCreation);
