...
import { Avatar, Badge, Button, Card, Col, ConfigProvider, Divider, Dropdown, Empty, Icon, Menu, Progress, Popconfirm,Row, Spin, Table, Typography } from 'antd'
...
const customizeRenderEmpty = () => (
  <>
    <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No Events">
      <Button href="/create" type="primary">Create Event</Button>
    </Empty>
  </>
);
...
const getGross = (event) => {
    ...
    event.tickets.forEach((ticket)=>{
        for (let ticketType in ticket.metadata){
          if ((['firstName','lastName','emailAddress', 'phoneNumber', 'eventId', 'total', 'updatedAt','status', 'guestId', 'title'].indexOf(ticketType) > -1)) continue
  ...
  render(){
    ...
    const desktopColumns = [
      {
        ...
        render: (text, event) => {
          ...
          <>
            <Row gutter={8}>
              <Col span={7} >
                <Avatar shape="square" size={64} src={event.image} icon="user" />
              </Col> 
              <Col span={17} >
      ...
      },
      {
        ...
        render: (text, event) => {
            let status = ''
            if (new Date(event.startDate).getTime() >new Date().getTime() && event.eventStatus !== 'draft')
      ...
      },
      {
        ...
        render: (text, event) => {
          const menu = (
            <Menu >
              <Menu.Item key={`/e/${event.slug}`}><Link href={`/e/${event.slug}`}>View</Link></Menu.Item>
              <Menu.Item key={`/manage/${event._id}`}><Link href={`/manage/${event._id}`}>Manage</Link></Menu.Item>
              <Menu.Item  onClick={()=> {
    ...
    const mobileColumns = [
      ...
      },
      {
        ...
        key: 'name',
        render: (text, event) => {
            console.log(event.title, event.startDate, new Date(event.startDate).getTime(), new Date().getTime() )
            let status = ''
            if (new Date(event.startDate).getTime() >new Date().getTime() && event.eventStatus !== 'draft')
              status = <Badge status="success" text="Live" />
            if (event.eventStatus === 'draft')
    ...
    return (
      <> 
        <Card title="Events" extra={<Button type="primary" href="/create">Create Event</Button>}>
